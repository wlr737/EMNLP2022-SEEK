...
def evaluate(model, data, ty="valid", max_dec_step=30):
    ...
    for j, batch in pbar:
        loss, ppl, cls_loss, accs, top_preds, comet_res = model.train_one_batch(batch, 0, train=False)
        ...
        pbar.set_description(
            "loss:{:.4f} ppl:{:.1f}".format(np.mean(l), math.exp(np.mean(ppl_a)))
        )
    ...
    if ty == "test":
        ...
        return np.mean(l), math.exp(np.mean(ppl_a)), np.mean(dia_a), np.mean(utt_a), np.mean(
            trg_a), results, mi_dist1, mi_dist2, avg_len
    return np.mean(l), math.exp(np.mean(ppl_a)), np.mean(dia_a), np.mean(utt_a), np.mean(trg_a), results
