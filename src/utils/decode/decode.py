...
class Translator(object):
    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        with torch.no_grad():
            ...
            ctx_output = enc_outputs.transpose(1, 2).reshape(dim0_enc, dim1_enc * dim2_enc, -1)
            src_mask = src_mask.transpose(1, 2).reshape(dim0_enc, 1, -1)



            dim0_enc, dim1_enc, dim2_enc = enc_outputs.size()[0], enc_outputs.size()[1], enc_outputs.size()[2]
            src_enc = enc_outputs.transpose(1, 2).reshape(dim0_enc, dim1_enc * dim2_enc, -1)
            src_mask = src_mask.transpose(1, 2).reshape(dim0_enc, 1, -1)
            ...
            src_seq = batch[0].repeat(1, n_bm).view(n_inst * n_bm, len_s)
